...
在入门教程中，我们会采用 Redux repo 中的计数器示例来作为演示。这个示例应用非常基础，但却很好的展示了 redux-saga 中的一些基础概念，且不会让你面对太多的技术细节而感到迷失。

### The initial setup

Before we start, clone the [tutorial repository](https://github.com/redux-saga/redux-saga-beginner-tutorial).

在开始之前，我们先克隆 [tutorial repository](https://github.com/redux-saga/redux-saga-beginner-tutorial)。
...
我们从最基础的使用场景来作为切入点：2 个按钮，一个 `Increment` 用来增加计数，`Decrement` 用来减少计数。然后，我们将会介绍异步调用。

If things go well, you should see 2 buttons `Increment` and `Decrement` along with a message below showing `Counter: 0`.

如果一切进展顺利，你应该会看到 2 个按钮，即 `Increment` and `Decrement`，在按钮下面还有一个提示 `Counter: 0`。
...
>
> 如果你在运行应用的时候遇到了问题。请在 [tutorial repo](https://github.com/redux-saga/redux-saga-beginner-tutorial/issues) 创建一个 issue。

## Hello Sagas!

We are going to create our first Saga. Following the tradition, we will write our 'Hello, world' version for Sagas.

我们马上就要创建第一个 Saga 了。遵照惯例（一般以 “Hello，world” 为第一个示例），我们将编写一个 Saga 版本的 “Hello，world”。
...
没啥好怕的，这就是一个普通函数（除了`*`）。它所完成的操作就仅是在控制台输出一条消息。
...
- 使用需要运行的 Sagas 列表来创建一个 Saga 中间件（目前我们只有一个 `helloSaga`）。
- connect the Saga middleware to the Redux store
- 把 Saga 中间件与 Redux store 进行关联
...
首先我们从 `./sagas` 模块 import 我们的 Saga。然后使用 `redux-saga` 提供的 `createSagaMiddleware` 来创建一个 Saga 中间件。
...
在运行 `helloSaga` 之前，我们必须用 `applyMiddleware` 来将中间件与 Store 进行关联。然后我们可以使用 `sagaMiddleware.run(helloSaga)` 来启动我们的 Saga。

So far, our Saga does nothing special. It just logs a message then exits.

目前为止，我们的 Saga 没有做任何特别的操作。它仅仅在输出一条消息之后就退出了。
...
现在我们来添加一些更接近原始计数器示例的代码。为了说明异步调用，我们需要添加另一个按钮用于在它被点击 1 秒之后增加计数。

First things first, we'll provide an additional button and a callback `onIncrementAsync` to the UI component.

第一步，我们先在 UI 组件中添加额外的按钮和该异步按钮被点击之后需要执行的回调函数 `onIncrementAsync`。 
...
接下来我们需要把 `onIncrementAsync` 和 Store 的 action 联系起来。
...
我们创建了一个 `delay` 函数，它返回一个将在指定毫秒之后变为 resolve 状态的 [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise)。我们将使用这个函数来 *中断* Generator。
...
Saga 使用 [Generator 函数](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/function*) 来作为实现，它 *yield* 的对象会到达 redux-saga 中间件。这些 yield 的对象是能够被中间件理解的一些指令。当一个 Promise 被 yield 给中间件，中间件就会暂停 Saga 直到 Promise 完成。在上面的例子中， `incrementAsync` Saga 被暂停直到 `delay` 返回的 Promise 变为 resolve 状态，这个操作会花上 1 秒钟。
...
一旦 Promise 变为 resolve 状态，中间件将恢复 Saga，并执行到下一个 yield 的位置。在这个例子中，下一个语句是另一个 yield 的对象：`put({type: 'INCREMENT'})` 的返回结果，它用于指示中间件需要 dispatch 一个 `INCREMENT` action。
...
`put` 是一个被我们称作 *Effect* 的示例。Effects 是纯 JavaScript 对象，其中包含中间件需要完成的指令。当中间件检测到一个 Saga yield 出来的 Effect 时，Saga 就会暂停直到 Effect 完成。 
...
总而言之， `incrementAsync` 通过调用 `delay(1000)` 来休眠 1 秒，然后 dispatch 一个 `INCREMENT` action。 
...
接下来，我们创建另一个 Saga `watchIncrementAsync`。我们使用 `takeEvery`，它是 `redux-saga` 提供的一个函数，它会监听每一次对 `INCREMENT_ASYNC` action 的 dispatch 操作并执行 `incrementAsync`。
...
现在我们有两个 Saga ，我们需要同时启动它们。我们需要添加一个 `rootSaga` 来负责启动其他 Saga。像下面这样重构 `sagas.js`：
...
该 Saga yield 了一个包含执行另外两个 saga 返回结果的数组，即 `helloSaga` 和 `watchIncrementAsync`。这意味着这两个 Generator 将会并行启动。现在我们只需要在 `main.js` 中对 root Saga 调用 `sagaMiddleware.run`。
...
我们希望能对 `incrementAsync` 进行测试，以确保它能按照预期执行。
...
`incrementAsync` 是一个 Generator 函数。当它运行的时候，它返回一个 iterator 对象，并且该 iterator 的 `next` 方法返回一个具有如下结构的对象
...
`value` 字段包含 yield 出来的表达式，如 `yield` 后面的表达式的结果值。`done` 字段表明该 Generator 是否仍然还有 'yield' 表达式或者已经终止。
...
因此如果我们连续调用该 Generator 3 次，我们将会得到如下结果：
...
前面 2 次返回 yiled 表达式的结果。由于第三次调用时没有更多 yiled 因此 `done` 字段被置为 true。并且由于 `incrementAsync` 没有任何返回值（即没有 `return` 语句），`value` 字段被置为 `undefined`。
...
所以现在，为了测试 `incrementAsync` 内部的逻辑，我们不得不对 Generator 的返回值进行迭代并检查它 yield 的 value。
...
现在的问题是我们如何测试 `delay` 的返回值？我们并不能对 Promise 做简单的等价测试。如果 `delay` 返回一个 *普通* 值，事情就变得简单得多了。
...
好的，`redux-saga` 提供了一种能够让上面描述的问题变得很好解决的方式。我们不能直接在 `incrementAsync` 内部调用 `delay(1000)`，而是 *间接* 调用它并将它导出以便让接下来的深层次比较变得可能：
...
相比于 `yield delay(1000)`，取而代之的是使用 `yield call(delay, 1000)` 。有什么不同？
...
在第一种情况下， yield 表达式 `delay(1000)` 在它被传递给 `next` 的调用者之前就被执行了（当运行我们现在的代码时，调用者可能是中间件，也可能是我们用于执行 Generator 函数并迭代其返回值的测试代码）因此调用者得到的是一个 Promise，就像上面的 test 代码中那样。
...
在第二种情况下，表达式 `call(delay, 1000)` 的值就是要传递给 `next` 调用者的。`call` 就像 `put` 那样，返回一个 Effect，它指示中间件使用指定的参数来调用指定的方法。实际上，不管是 `put` 还是 `call` 他们本身都不执行任何 dispatch 或者异步调用，它们只是返回了一个纯 JavaScript 对象。
...
中间件会检查每个 yield 后面的 Effect 的类型然后决定如何来完成这个 Effect。如果 Effect 类型是 `PUT` 则它会 dispatch 一个 action 给 store。如果 Effect 类型是 `CALL` 则它会调用这个指定的函数。 
...
这种将 Effect 创建和 Effect 执行分开的做法使得我们对 Generator 代码的测试变得异常简单：
